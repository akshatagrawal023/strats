...
class OptionToMatrix:
    ...
    def create_matrix_from_response(self, resp):
        ...
        for i in range(1, len(options), 2):
            ...
            mat[2, si] = ce_row.get('volume', np.nan)
            mat[3, si] = ce_row.get('oi', np.nan)
            mat[4, si] = ce_row.get('oich', np.nan)
            mat[5, si] = pe_row.get('bid', np.nan)
            mat[6, si] = pe_row.get('ask', np.nan)
            mat[7, si] = pe_row.get('volume', np.nan)
            mat[8, si] = pe_row.get('oi', np.nan)
